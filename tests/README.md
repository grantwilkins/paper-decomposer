...
| [`test_extraction_heavy_cleanup.py`](test_extraction_heavy_cleanup.py) | Enabled heavy cleanup runs after deterministic normalization, respects model-call budgets, and remains behind final validation. | No |
...
| [`test_extraction_stages.py`](test_extraction_stages.py) | Extraction stages map `cheap` to `small`, keep compression/repair on configured cheap tiers, route cleanup to heavy adjudication, and include evidence span IDs in prompts. | No |
